---
name: software-engineering-nextjs-cache
description: Use this skill any time you're fetching data in a Next.js 16 app, deciding what to cache, designing Suspense boundaries, or troubleshooting why a page is slow or shows stale data. Triggers include the "use cache" directive, Cache Components, cacheLife, cacheTag, the cacheComponents config flag, fetch() inside server components, parallel data fetching, request waterfalls, <Suspense> boundaries, loading.tsx, streaming, revalidatePath, or revalidateTag. Also use when designing the per-endpoint caching strategy for a new feature, when an endpoint that should be dynamic is returning stale data (or vice versa), or when verifying that Cache Components are enabled in the project config.

# Project context
type: skill
project: skills-library
plugin: software-engineering
aliases: [software-engineering-nextjs-cache]
tags: [type/skill, plugin/software-engineering, topic/nextjs, topic/caching, topic/performance]
status: active
---
...
This is the recommended default for new Next.js 16 projects.
...
export async function getCart(token: string) {
  "use cache";
  cacheTag(`cart:${token}`);
  return fetch(`${API}/cart`, { headers: { "x-cart-token": token } }).then((r) => r.json());
}
...
4. **Cache the static, stream the dynamic.** This is the Cache Components mantra.

## A typical caching decision matrix
...
- Using `"use cache"` on cart / auth code — the wrong call here could leak one user's data to everyone.
- `Promise.all`-ing two fetches when they should be in separate Suspense boundaries (one fast, one slow → fast one stalls).

## How this skill relates to others

- Where Suspense and loading.tsx files belong → [[software-engineering-nextjs-routing]]
- Server vs client decisions for the components inside Suspense → [[software-engineering-nextjs-server-client]]
- Mutations that should `revalidatePath` / `revalidateTag` → [[software-engineering-nextjs-actions]]
- error.tsx behavior when a Suspense'd fetch throws → [[software-engineering-nextjs-errors]]
- Web Vitals impact of caching choices → [[software-engineering-nextjs-performance]]
